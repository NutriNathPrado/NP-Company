// Gera uma SEQUÊNCIA DE STORIES do dia a dia (misto câmera/tela), na voz da Nathalia.
...
export async function POST(req: Request) {
  ...
  const reguaBlock = `\n\nRÉGUA DA MARCA:\nPÚBLICO: ${aud}\nARESTA/CARA: ${edg}`;
  const histBlock = model.historia?.trim() ? `\n\nVIDA REAL DA NATHALIA (use SÓ quando o tema pedir algo pessoal/bastidor; NUNCA invente):\n${model.historia.trim().slice(0, 3000)}` : "";
  const goldBlock = gold.length ? `\n\nA VOZ DA NATHALIA (imite a cadência, NÃO copie):\n${pickRandom(gold, 2).map((g) => g.text).join("\n---\n")}` : "";
  const rejectBlock = rejects.length ? `\n\nFOGE DESSE PADRÃO (a Nathalia odeia: anti-ouro):\n${rejects.slice(0, 6).map((r) => `✗ ${r.text}`).join("\n")}` : "";
  const regBlock = registroBlock(body.registro);
  const estiloBlock = estilo ? `\n\nESTILO DE REFERÊNCIA DOS STORIES (siga ESTE estilo de escrita e formato: é o jeito que a Nathalia curte fazer story, calibrado pelos prints e pelo arquivo base dela; PRIORIZE isto sobre suposições):\n${estilo}` : "";
  ...
  try {
    ...
    for (let attempt = 0; attempt < 3 && !parsed; attempt++) {
      ...
      if (tells.length && attempt < 2) {
        retryNote = `\n\nATENÇÃO: a versão anterior tinha cara de IA (${tells.slice(0, 4).join(" | ")}). Refaz mais humano e mais Nathalia: SEM "não é A, é B", sem espelho "não é sobre X é sobre Y", sem clichê de coach, sem floreio e sem travessão.`;
        continue;
      }
