...
export default function InstagramMetricsPanel({ snapshot, compact = false }: { snapshot: InstagramDashboardSnapshot; compact?: boolean }) {
  ...
  const postViews7 = useMemo(() => sumAvailable(posts7, "views"), [posts7]);
  const postReach30 = useMemo(() => sumAvailable(posts30, "reach"), [posts30]);
  const postReach7 = useMemo(() => sumAvailable(posts7, "reach"), [posts7]);
  const views30 = hasNumber(snapshot.account?.views30) ? snapshot.account.views30 : postViews30;
  const views7 = hasNumber(snapshot.account?.views7) ? snapshot.account.views7 : postViews7;
  const reach30 = hasNumber(snapshot.account?.reach30) ? snapshot.account.reach30 : postReach30;
  const reach7 = hasNumber(snapshot.account?.reach7) ? snapshot.account.reach7 : postReach7;
  const accountTotalsAvailable = hasNumber(snapshot.account?.views30) || hasNumber(snapshot.account?.reach30);
      ...
      <section className={compact ? "ig-metrics-panel ig-metrics-panel--compact" : "ig-metrics-panel"} aria-labelledby={compact ? "metricas-hoje-title" : "metricas-title"}>
        <div className="ig-section-head ig-section-head--plain">
          <div>
            ...
            <p>{accountTotalsAvailable ? "Totais gerais da conta no período, atualizados pela Meta." : "* Totais provisórios: soma dos posts datados lidos pela API."}</p>
          </div>
        </div>
        <div className="ig-metrics-grid">
          ...
          <MetricCard label={`Visualizações · últimos 30 dias${hasNumber(snapshot.account?.views30) ? "" : "*"}`} value={formatNumber(views30)} />
          <MetricCard label={`Visualizações · últimos 7 dias${hasNumber(snapshot.account?.views7) ? "" : "*"}`} value={formatNumber(views7)} />
          <MetricCard label={`Alcance · últimos 30 dias${hasNumber(snapshot.account?.reach30) ? "" : "*"}`} value={formatNumber(reach30)} onClick={() => setInfoModal("reach")} />
          <MetricCard label={`Alcance · últimos 7 dias${hasNumber(snapshot.account?.reach7) ? "" : "*"}`} value={formatNumber(reach7)} onClick={() => setInfoModal("reach")} />
